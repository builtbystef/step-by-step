...
Planning. There is no code yet: the six product areas are specced and the decisions are recorded, but the stack has not landed. See the roadmap (`.beaver/issues/idnzwf-*.md`) for what is settled and what is still open.

The intended shape: a Next.js/TypeScript frontend, a FastAPI/Python backend, versioned Workflows in PostgreSQL, Redis-queued Runs on isolated Playwright Workers, and object storage for artifacts — one docker compose stack.
