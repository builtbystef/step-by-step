...
- `step_by_step_core.events` — the Run event vocabulary. `publish` writes one message to `run:{id}:events`; `publish_log` is the dual-write helper that inserts a `run_log_lines` row and publishes the matching `log` event, capping a Run at 10 000 lines plus one final `log truncated` line. An `artifact` event carries ids only.
...
`routes.py` starts manual and test Runs, lists them by a newest-first `(queued_at, id)` keyset, reconstructs detail from the Run, ordered Step Results, and control intervals, streams live events over SSE, returns persisted log lines, and cancels queued Runs immediately. `GET /api/runs/{id}/events` checks Organization membership before it subscribes — another Organization's id is 404 and never opens a Redis subscription — then fans out `run:{id}:events` with no replay and no `Last-Event-ID`. `GET /api/runs/{id}/logs` is the same gate over `run_log_lines`, filterable by `after_seq` and `step_id`. `GET /api/attention` uses one scan of that partial index: windowed filtered counts retain the true queued, running, and waiting totals while a waiting-first limit returns no more than the five soonest takeover deadlines and their Workflow names. Every lookup passes through the active-Organization Membership gate, so an id from another Organization is 404. Starting commits the queued row to Postgres first, then performs one `LPUSH` of its id to `step_by_step_core.bus.DISPATCH_LIST`; Redis is a dispatch hint, never the record of whether the Run exists. Active cancellation, Worker claims, Artifact rows, and Batch rows arrive in their owning execution slices, so detail returns empty Artifacts and no Batch row until those stores exist.

### Schedules and the minute loop

`step_by_step_api.schedules` owns cron triggers. A Schedule belongs to one Workflow and its Organization, stores an IANA timezone, and computes `next_due_at` with croniter (timezones stay on the standard library's `zoneinfo`). The CRUD routes scope every id through the active-Organization Membership gate; another Organization's Schedule is 404. Creating or turning a Schedule back on sets `next_due_at` to the next future occurrence, so a disable/enable never catches up missed ones.

`step_by_step_api.loop.tick` is the one directly-invokable pass of the backend's minute loop. This slice's duty is firing: each enabled Schedule whose current occurrence has passed creates a Run of the Workflow's latest published Version (`trigger` = `schedule`, no starter) and `LPUSH`es it after the Postgres commit. A still-non-terminal Run of that same Schedule records `last_skip_reason` = `overlap` instead. If a later occurrence of the same expression has also passed, every missed one is skipped and `next_due_at` jumps to the next future instant. Reaping duties (stale heartbeats, takeover deadlines, the queued-Run backstop) join this same `tick` rather than starting a second loop. Tests call `tick()` themselves; the process starts the minute waiter from the app lifespan.
...
Each claimed Run gets a fresh headed persistent Chromium context under a temporary profile directory. The executor parses either the immutable Version document or a test Run's stored Draft snapshot through `step_by_step_core.document`, walks Steps in order through the selector-resolution seam, and commits each Step Result before touching the next Step. It emits `step.started` / `step.finished` as it walks and a terminal `run.status` when it finishes, and each walked Step also produces a log line through the dual-write helper. One automation control interval covers this slice's Run; disabled and optional-missing Steps are skipped, required failure skips every unreached Step, and the accumulated automation clock is checked at Step boundaries. Terminal cleanup closes Chromium and removes the profile before writing the final status. Credentials, takeover control, Artifacts, and heartbeats after claim plug in through their owning slices.
