...
`apps/web` is Tailwind CSS 4 with shadcn/ui generated against Radix, and TanStack Query for server state. The vocabulary lives in four places, and a screen inherits it rather than inventing one:
...
The shadcn CLI is run with `pnpm --config.trustPolicy=none dlx shadcn@latest add <component>`. The relaxation covers the generator's own transient dependency tree only — it never reaches the lockfile — and it is needed because shadcn's tree pulls `semver@6.3.1`, a legacy version with no provenance, which the workspace's `trustPolicy: no-downgrade` refuses. Do not let the CLI add `shadcn` itself as a project dependency; it is a generator, not a runtime.
