...
- `slots.tsx` is the three places a later slice fills: the attention band and the Runs count badge (`fkgat7`), and the extension's connection pill (`20k5ft`). They are components rather than gaps in the markup, so the slice that arrives replaces one function body.
