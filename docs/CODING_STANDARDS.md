...
Defined by spec `pc0t8s` and built in `8cjj8g`. Two rules a review checks against a diff:
...
Both are also asserted by `apps/web/visual-language.test.ts`, which scans the frontend's own source.
