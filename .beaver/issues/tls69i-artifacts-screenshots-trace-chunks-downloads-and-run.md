...
Everything a Run leaves behind. The `artifacts` table (run, step, kind screenshot | trace | download, object key, content type, size, index); Workers write objects to MinIO directly and insert the rows themselves — bytes never pass through the backend — and announce each with an `artifact` event.
...
DELETE /api/runs/{id} → 204 (terminal only; purges rows and MinIO objects)
                        409 code=run_active
```
...
- [ ] A Step that downloads a file → a `download` Artifact with the suggested filename and content type, and the bytes in MinIO match the fixture file.
...
- [ ] `DELETE` on a `running` Run → 409 `run_active`; on a terminal Run → 204, and the Run's rows, Step Results, log lines, and MinIO objects are all gone (verified by observing the object store).
